...
import { getStory } from '../storyActionCreators';
...
const Story = ({ story }) => {
    ...
    // Function to handle opening the story details page
    const handleOpen = () => {
    ...
    const editStory = (e) => {
        ...
        e.preventDefault();
        dispatch(closeModal());
        dispatch(openModal('UPDATE_STORY'));
        dispatch(getStory(story._id, userId));
    };

    //Retrieve Image Url from Redux State
    const storyImg = story.slides[0]?.imageUrl;

    return (
        <>
            {/* Story Card Container */}
            <div className={styles.storyCardCon}>
                {/* Card Image , Heading , Description Div */}
                <div
                    className={styles.storyDiv}
                    style={{
                        backgroundImage: `linear-gradient(#00000000, #00000099), url(${storyImg}) `,
                        backgroundSize: 'cover',
                    }}
                    onClick={handleOpen}
                >
                    <div className={styles.storyHeadingDescCon}>
                        {/*Story Heading */}
                ...
                {/* Render edit button if authenticated user is the story owner */}
                {isAuthenticated && userId && story.addedBy === userId && (
                    <div
                        className={styles.editStorySvg}
                        onClick={editStory}
